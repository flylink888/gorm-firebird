...
func (m Migrator) ColumnTypes(value interface{}) ([]gorm.ColumnType, error) {
	...
	err := m.RunWithValue(value, func(stmt *gorm.Statement) error {
		var
		columnTypeSQL   = "SELECT LOWER(B.RDB$FIELD_NAME) column_name, B.RDB$NULL_FLAG is_nullable, (CASE D.RDB$TYPE_NAME  WHEN 'TEXT' THEN 'CHAR' WHEN 'INT64' THEN 'BIGINT' WHEN 'LONG' THEN IIF(C.RDB$FIELD_SCALE=0,'INTEGER','NUMBERIC') WHEN 'SHORT' THEN 'SMALLINT' WHEN 'DOUBLE' THEN 'DOUBLE' WHEN 'VARYING' THEN 'VARCHAR' WHEN 'FLOAT' THEN 'FLOAT' WHEN 'BLOB' THEN 'BLOB' WHEN 'TIMESTAMP' THEN 'TIMESTAMP' END) data_type, IIF(D.RDB$TYPE_NAME='VARYING',C.RDB$FIELD_LENGTH/4,C.RDB$FIELD_LENGTH*4) character_maximum_length, C.RDB$FIELD_PRECISION numeric_precision, C.RDB$FIELD_SCALE numeric_scale FROM RDB$RELATIONS A INNER JOIN RDB$RELATION_FIELDS B ON A.RDB$RELATION_NAME = B.RDB$RELATION_NAME INNER JOIN RDB$FIELDS C ON B.RDB$FIELD_SOURCE = C.RDB$FIELD_NAME INNER JOIN RDB$TYPES D ON C.RDB$FIELD_TYPE = D.RDB$TYPE WHERE A.RDB$SYSTEM_FLAG = 0 AND D.RDB$FIELD_NAME = 'RDB$FIELD_TYPE' AND A.RDB$RELATION_NAME = 'USERS' ORDER BY B.RDB$FIELD_POSITION "
